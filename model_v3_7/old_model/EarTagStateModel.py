...
from old_code.report import send_error_report
...
class EarTagStateModel(object):
    ...
    def backtrack_for_detect(self, curDateTime, backtrack_data, backtrack_markIdList):
        ...
        print('耳标状态判别模型')
        print('去重数据量:', len(np.unique(backtrack_markIdList)), '总数据量:', len(backtrack_markIdList))
        ...
        # 获得脱落待修正的markId
        need_judge_tag_off_data = "SELECT mark_id FROM {} WHERE status = 20".format(EAR_TAG_MODEL_TABLE)
        ...
        for markId in tqdm(np.unique(backtrack_markIdList)):
            ...
            try:
                self.__tag_off_judgement(backtrack_data)
            except:
    ...
    def __ear_tag_judgement(self, backtrack_data):
        ...
        #     self.__tag_off_amendment(backtrack_df)
        #     return

        # # 脱落待定模型
        # if str(self.markId) in self.undetermined_tag_off_list:
        #     self.__tag_off_undetermined(backtrack_df)
        #     return
        ...
        back_ago_df = backtrack_df.head(TAG_OFF_PERIOD * 60)  # 获取最前一小段数据
        ...
        if np.isnan(mean_env_temp):  # 环境温度为空时,跳出判别
            ...
            return
        temp_threshold = mean_ear_temp - mean_env_temp - TAG_OFF_TEMP_THRESHOLD  # 耳标脱落阈值
        temp_threshold = np.where(temp_threshold > MINIMUM_TEMP_CHANGE_THRESHOLD, temp_threshold,
                                  MINIMUM_TEMP_CHANGE_THRESHOLD)  # 设置最低脱落差阈值
        ...
        temp_mean = np.mean(new_df['temperature_earTag'])  # 耳标温度均值
        # temp_diff = temp_mean - mean_env_temp  # 温度差均值
        # temp_change = np.median(old_df['temperature_earTag']) - np.median(new_df['temperature_earTag'])  # 温度变化量  v1
        # temp_change = np.max(old_df['temperature_earTag']) - np.min(new_df['temperature_earTag'])  # 温度变化量  v3.5
        # old_df.reset_index(inplace=True, drop=True)
        # new_df.reset_index(inplace=True, drop=True)
        # temp_change = old_df.iloc[0][ 'temperature_earTag'] - old_df.iloc[-1][ 'temperature_earTag']  # 温度变化量  v2修改版 脱落后数据缺失容易漏判
        # temp_change = old_df.iloc[0]['temperature_earTag'] - new_df.iloc[0]['temperature_earTag']  # 温度变化量  v3修改版
        # if 7 < self.curDateTime.hour < 18 and self.type_conf_id in TYPE_OFF_COW_LIST:  # 对于牛处于中午的情况额外处理
        #     temp_change = old_df.iloc[0]['temperature_earTag'] - np.min(new_df['temperature_earTag'])  # 温度变化量  v3.4修改版
        ...
        if sport_rate > SPORT_RATE_THRESHOLD:  # 如果运动量较低
            ...
            if str(self.markId) in self.not_wear_data_list:
                ...
                self.status = 0
                self.__ear_tag_to_mysql()
                self.__param_insert_to_mysql(temp_threshold=temp_threshold,
                                             sport_max=sport_max,
                                             temp_diff=diff_rate, sport_rate=sport_rate, temp_max=temp_max,
                                             temp_min=temp_min,
                                             mean_env_temp=mean_env_temp, temp_mean=temp_mean, remark=0,
                                             output_node='更新未佩戴时间')
                return
            temp_change = tag_off_feature.extremum_temp_change_with_pried(pried=20,
                                                                          interval=10)  # 每20分钟温度变化量最大值  v3.5.2
            max_temp_change = tag_off_feature.maximum_rate_of_temp_change()  # V3.5.1添加

            temp_change = np.where(temp_change > max_temp_change, temp_change, max_temp_change)

            # 脱落判别
            if (
                    temp_change / temp_threshold > TAG_OFF_TEMP_RATE and max_temp_change / temp_threshold > MAX_TEMP_CHANGE_RATE) or max_temp_change > MAX_TEMP_CHANGE_THRESHOLD:
                self.ai_remark = 2
                self.status = 2
                self.__ear_tag_to_mysql()
                self.__param_insert_to_mysql(temp_change=temp_change, temp_threshold=temp_threshold,
                                             sport_max=sport_max,
                                             temp_diff=diff_rate, sport_rate=sport_rate, temp_max=temp_max,
                                             temp_min=temp_min, max_temp_change=max_temp_change,
                                             mean_env_temp=mean_env_temp, temp_mean=temp_mean, remark=2,
                                             output_node='脱落过程判断')
            ...
            if back_sport_zero > 0.98:
                ...
                self.__param_insert_to_mysql(temp_change=temp_change, temp_threshold=temp_threshold,
                                             ...
                                             temp_diff=diff_rate, sport_rate=sport_rate, temp_max=temp_max,
                                             temp_min=temp_min, max_temp_change=max_temp_change,
                                             mean_env_temp=mean_env_temp, temp_mean=temp_mean, remark=-1,
    ...
    def __tag_off_judgement(self, backtrack_data):
        if str(self.markId) in self.destroy_and_ear_tag_off_list:
            ...
            return
        backtrack_df = backtrack_data[backtrack_data['markId'] == self.markId].copy()
        new_df = backtrack_df.tail(TAG_OFF_SEQ_LEN)
        new_df.reset_index(inplace=True, drop=True)
        if len(new_df) == 0:
            if str(self.markId) in self.need_judge_tag_off_list and self.curDateTime.hour > DAY_TIME:
                self.status = 2
        ...
        if str(self.markId) in self.need_judge_tag_off_list:
            query_update_time = "SELECT update_time FROM {} WHERE mark_id = {} ORDER BY update_time DESC LIMIT 1".format(
                EAR_TAG_MODEL_TABLE, self.markId)
            ...
            if p1 > TAG_OFF_JUDGE_RATE:
                self.status = 2
                self.__param_insert_to_mysql(output_node='符合脱落特征，跳出修正', p0=p0, p1=p1, p2=p2, p3=p3)
                if self.to_mysql:
                    self.__ear_tag_update_to_mysql()
            elif (result == 2 or result == 3) and DAY_TIME <= self.curDateTime.hour <= NIGHT_TIME:
                self.status = 2
                self.__param_insert_to_mysql(output_node='脱落判别成功，跳出修正', p0=p0, p1=p1, p2=p2, p3=p3)
                if self.to_mysql:
                    self.__ear_tag_update_to_mysql()
            elif self.curDateTime - timedelta(hours=DEATH_PIG_AMENDMENT_PRIED) > update_time:
                ...
                elif p1 > TAG_OFF_ADJUST_RATE:
                    self.__param_insert_to_mysql(output_node='符合脱落过程特征，不做修正', p0=p0, p1=p1, p2=p2, p3=p3)
                else:
                    self.status = 1
            ...
            # else:
            self.status = 2
            self.__param_insert_to_mysql(output_node='脱落判别成功', p0=p0,  p1=p1, p2=p2, p3=p3)
            if self.to_mysql:
                self.__ear_tag_to_mysql()

        elif result == 1:
            self.status = 20
            self.__param_insert_to_mysql(output_node='符合脱落特征，进入脱落修正判别', p0=p0, p1=p1, p2=p2, p3=p3)
            if self.to_mysql:
                self.__ear_tag_to_mysql()
        elif result == 3:
            tag_off_feature = EarTagModelFeature(old_df=new_df)
            max_temp_change = tag_off_feature.maximum_rate_of_temp_change()  # V3.5.1添加
            if max_temp_change > MAX_TEMP_CHANGE_THRESHOLD:
                self.status = 20
                self.__param_insert_to_mysql(output_node='达到温度变化阈值，进入脱落修正判别', p0=p0, p1=p1, p2=p2, p3=p3, max_temp_change=max_temp_change)
                if self.to_mysql:
                    self.__ear_tag_to_mysql()
            else:
                self.status = 1
                self.__param_insert_to_mysql(output_node='未达到温度变化阈值，不符合脱落判别', p0=p0, p1=p1, p2=p2, p3=p3, max_temp_change=max_temp_change)
                if self.to_mysql:
                    self.__ear_tag_to_mysql()
