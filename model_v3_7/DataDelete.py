...
class DataDelete(object):
    ...
    def delete_product_data(self):
        ...
        self.conn.commit()
        delete_ear_tag_undermined = "DELETE FROM {} ;".format(EAR_TAG_UNDETERMINED_TABLE)
        self.cursor.execute(delete_ear_tag_undermined)
        self.conn.commit()

    def delete_ear_tag_param_data(self, last_date_time=None):
    ...
    def delete_death_param_data(self, last_date_time=None):
        query_earliest_datetime = "SELECT MIN(update_time) FROM {}".format(DEATH_PARAM_TABLE)
        query_longest_datetime = "SELECT MAX(update_time) FROM {}".format(DEATH_PARAM_TABLE)
        earliest_datetime = np.array(pd.read_sql(query_earliest_datetime, con=self.conn))[0][0]
        longest_datetime = np.array(pd.read_sql(query_longest_datetime, con=self.conn))[0][0]
        if earliest_datetime is None:
            return
...
if __name__ == '__main__':
    ...
    obj.delete_ear_tag_param_data()
    obj.delete_death_param_data()
    obj.close()
